...
#ifndef __MAST_bernoulli_beam_elem_h__
#define __MAST_bernoulli_beam_elem_h__
...
namespace MAST {
    class BernoulliElem {
    public:
        BernoulliElem(const Elem& elem, QBase& qrule):
        _global_elem(elem),
        _length(elem.volume())
        { }
         ...
         */
        void initialize_bending_strain_operator (const Point& p,
...
inline void
MAST::BernoulliElem::initialize_bending_strain_operator (const Point& p,
                                                         FEMOperatorMatrix& Bmat)
{
    const Real xi = p(0);
...
#endif // __MAST_bernoulli_beam_elem_h__
