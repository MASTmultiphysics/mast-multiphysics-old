...
_local_elem(elem),
_timoshenko_beam_shear_quadrature_reduction(2)
{
    ...
    if (bending_model == MAST::BERNOULLI)
        _bernoulli_elem.reset(new MAST::BernoulliElem(_local_elem.local_elem(),
                                                      *_qrule));
}
...
{
    FEMOperatorMatrix Bmat_bar, Bmat_bend, Bmat_trans, Bmat_vk;
    ...
    Bmat_bar.reinit(2, _system.n_vars(), n_phi);  // axial strain and torsion
    Bmat_bend.reinit(1, _system.n_vars(), n_phi); //
    Bmat_vk.reinit(1, _system.n_vars(), n_phi);   // only dw/dx and dw/dy
    ...
    if_bernoulli = (property_1D.bending_model(_elem, _fe->get_fe_type()) == MAST::BERNOULLI);
    ...
    for (unsigned int qp=0; qp<JxW.size(); qp++) {
        ...
        _internal_force_operation(if_bending, if_bernoulli, if_vk, n2, qp, JxW,
    ...
    // element
    if (if_bending && !if_bernoulli)
        _internal_timoshenko_beam_shear_force_operation
        (request_jacobian, local_f, local_jac,
                                            ...
                                            DenseMatrix<Real>& jac)
{
    ...
    for (unsigned int qp=0; qp<JxW.size(); qp++) {
        this->initialize_axial_strain_operator(qp, Bmat_mem);
        
        // get the bending strain operator if needed
        tmp_vec2_n1.zero(); // used to store vk strain, if applicable
        if (if_bending) {
            if (if_bernoulli)
                _bernoulli_elem->initialize_bending_strain_operator(qp, Bmat_bend);
            else
                                          ...
                                          DenseMatrix<Real>& jac)
{
    ...
    for (unsigned int qp=0; qp<JxW.size(); qp++) {
        this->initialize_axial_strain_operator(qp, Bmat);
...
void
MAST::StructuralElement1D::initialize_axial_strain_operator(const unsigned int qp,
                                                            FEMOperatorMatrix& Bmat) {
...
void
MAST::StructuralElement1D::initialize_timoshenko_bending_strain_operator(const unsigned int qp,
...
void
MAST::StructuralElement1D::_internal_timoshenko_beam_shear_force_operation
(bool request_jacobian,
 DenseVector<Real>& local_f,
 DenseMatrix<Real>& local_jac,
 DenseVector<Real>& tmp_vec3_n2,
 ...
 const MAST::SensitivityParameters* sens_params)
{
    ...
    qrule.reset(fe_type.default_quadrature_rule
                (_elem.dim(),
                 ...
                 - _timoshenko_beam_shear_quadrature_reduction).release());
...
bool
MAST::StructuralElement1D::surface_pressure_force_sensitivity(bool request_jacobian,
                                                              DenseVector<Real> &f,
                                                              DenseMatrix<Real> &jac,
                                                              const unsigned int side,
                                                              MAST::BoundaryCondition &p) {
