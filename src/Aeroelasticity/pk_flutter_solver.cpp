...
void
MAST::PKFlutterSolution::init (const MAST::FlutterSolverBase& solver,
                               const Real k_red,
                               const Real v_ref,
                               const Real bref,
...
MAST::PKFlutterSolver::scan_for_roots() {
    ...
    if (!_flutter_solutions.size()) {
        ...
        std::vector<Real> k_red_vals(n_k_red_divs+1);
        for (unsigned int i=0; i<n_k_red_divs+1; i++) {
            k_red_vals[i] = current_k_red;
            current_k_red -= delta_k_red;
        }
        k_red_vals[n_k_red_divs] = k_red_range.first; // to get around finite-precision arithmetic

        //
        //  outer loop is on reduced frequency
        //
        for (unsigned int j=0; j<n_k_red_divs+1; j++) {
