                                   ...
                                   FESystem::Numerics::VectorBase<FESystemDouble>& bc_vec, FESystem::Numerics::MatrixBase<FESystemDouble>& elem_mat)
{
    ...
    switch (case_type)
    {
        case NACA_AIRFOIL:
        {
            ...
            for ( ; it != end; it++)
            {
                fluid_elem.calculateSolidWallFluxBoundaryCondition(*it, q_boundary, surf_vel, tmp_vec);
                bc_vec.add(1.0, tmp_vec);
                
                if (if_calculate_jacobian)
                {
                    fluid_elem.calculateTangentMatrixForSolidWallFluxBoundaryCondition(*it, q_boundary, surf_vel, tmp_mat);
                    elem_mat.add(1.0, tmp_mat);
                }
            }

            bc_ids.clear();
            elem.getBoundariesWithTag(2, bc_ids); // outer boundary, to be treated with mixed boundary condition
            it = bc_ids.begin(), end = bc_ids.end();
            for ( ; it != end; it++)
            {
                fluid_elem.calculateMixedBoundaryCondition(*it, q_boundary, sol_inf, tmp_vec);
                bc_vec.add(1.0, tmp_vec);
                
                if (if_calculate_jacobian)
                {
                    fluid_elem.calculateTangentMatrixForMixedBoundaryCondition(*it, q_boundary, tmp_mat);
                    elem_mat.add(1.0, tmp_mat);
                }
            }
        }
...
public:
    ...
    void assemble(FESystemUInt e1, FESystemUInt e2) const
    {
        ...
        {
            n_elem_nodes = elems[i]->getNNodes();
            ...
            FESystem::Quadrature::initializeQRuleForElemBoundary(*(elems[i]), fe, q_boundary);
            elem_mat1.zero();
            elem_mat2.zero();
            elem_vec.zero();
                                                  ...
                                                  FESystem::Numerics::VectorBase<FESystemDouble>& tmp_vec) const
    {
        ...
        FESystem::FiniteElement::FELagrange fe;
        FESystem::Fluid::FluidElementBase fluid_elem;
        FESystem::Quadrature::initializeQRuleForElem(elem, fe, q_rule);
        FESystem::Quadrature::initializeQRuleForElemBoundary(elem, fe, q_boundary);
...
public:
    ...
    void assemble(const FESystemUInt n1, const FESystemUInt n2) const
    {
        ...
        tmp_vec.resize(n_vars); tmp_vec_complex.resize(n_vars);
...
public:
    ...
    void operator() (const tbb::blocked_range<FESystemUInt>& r) const
    {
        ...
        tmp_vec.resize(n_vars); tmp_vec2.resize(n_vars);
    ...
    while (!if_converged)
    {
        ...
        switch (call_back)
        {
                ...
                // the Jacobian is not updated since it is constant with respect to time
            {
                ...
                calculateEulerQuantities(dof_map, mesh, transient_solver.getCurrentStepSize(), transient_solver.getCurrentIterationNumber(),
...
void frequencyDomainAnalysis(const FESystem::Mesh::MeshBase& mesh, const FESystem::Base::DegreeOfFreedomMap& dof_map, const FESystem::Numerics::VectorBase<FESystemDouble>& final_sol)
{
    ...
    calculateEulerQuantities(dof_map, mesh, 1.0e3, 1000, final_sol, vel, true, res, stiff_mat, mass, additional_sol, additional_sol);
    
    // calculate the force vector
    tbb::parallel_for(tbb::blocked_range<FESystemUInt>(0, elems.size()),
                      EvaluateLinearizedForce(elems, 1.0e3, dof_map, final_sol, vel, dummy_vec, dc_vals, f_vec));
    ...
    additional_sol.zero();
    tbb::parallel_for(tbb::blocked_range<FESystemUInt>(0, nodes.size()),
                      EvaluateComplexCpPerturbation(elems, nodes, 1.0e3, dof_map, final_sol, vel, dc_vals, complex_sol, additional_sol));
...
//    createPlaneMesh(elem_type, mesh, origin, x_points, y_points, n_elem_nodes, CROSS, true);
    ...
    // transient solution
    transientEulerAnalysis(mesh, dof_map, sol_vec);
