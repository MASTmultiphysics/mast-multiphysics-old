...
// FESystem includes
#include "conduction/conduction_system.h"
                               ...
                               const std::string& system_name)
{
    ...
    system.project_solution(conduction_solution_value, NULL, es.parameters);
}
...
void ConductionSystem::init_data ()
{
    ...
    _k_thermal = infile("k_thermal", 0.);
    _cp_thermal = infile("cp_thermal", 0.);
    ...
    Tvar  = this->add_variable ( "Tvar", static_cast<Order>(o), fefamily);
    this->time_evolving(Tvar);
...
void ConductionSystem::init_context(DiffContext &context)
{
    ...
    c.get_element_fe( Tvar, elem_fe);
    elem_fe->get_JxW();
    elem_fe->get_phi();
    elem_fe->get_dphi();
    elem_fe->get_xyz();
    
    c.get_side_fe( Tvar, elem_fe);
    elem_fe->get_JxW();
    elem_fe->get_phi();
    elem_fe->get_xyz();
}
